...
class SyncManager {
  ...
  async pullFromServer(boardId) {
    ...
    // Check if we recently synced (within last 30 seconds)
    const lastSyncMeta = await db.syncMeta.get(`last_sync_${boardId}`);
    if (lastSyncMeta && (Date.now() - lastSyncMeta.value < 30000)) {
      console.log('Skipping pull - recently synced');
      return;
    }
